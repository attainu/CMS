...
module.exports = {
        ...
        try{
            ...
            if(place){
                ...
                if(gym.length === 0) return res.status(401).json({ statusCode:401, message: 'No gym present at this place' })
                 return res.status(200).json({ statusCode:200, membership: gym})
            }
        }catch(err){
    ...
    async findMSByCountry(req,res){
        try{
            ...
            return res.status(200).json({statusCode: 200, membership: arr1})
    ...
    async findMSByCity(req,res){
        try{
            ...
            return res.json({ statusCode: 200, membership: cityGym})
        }catch(err){
            return res.status(500).json({ statusCode:500, message: 'Server Error' })
        }
    }
